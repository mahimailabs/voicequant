...
class ServerConfig(BaseModel):
    ...
    model: str = Field(default="Qwen/Qwen2.5-7B-Instruct-AWQ", description="HuggingFace model ID")
    host: str = Field(default="0.0.0.0", description="Server bind host")
    port: int = Field(default=8000, description="Server bind port")

    # TurboQuant settings
    kv_cache_dtype: str = Field(default="tq4", description="KV cache dtype: tq4, tq3, fp16")
    ...
    gpu_memory_utilization: float = Field(default=0.90, description="GPU memory utilization")
