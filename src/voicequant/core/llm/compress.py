...
def turboquant_compress_kv_3bit(
    ...
    # keys
    k_tile = ct.load(
        K, index=(block_id, 0), shape=(BLOCK_S, HEAD_DIM), padding_mode=zero_pad
    )
    ...
    # values
    v_tile = ct.load(
        V, index=(block_id, 0), shape=(BLOCK_S, HEAD_DIM), padding_mode=zero_pad
    )
